...
<html>
    ...
    <body>
        ...
        <?php if($_SESSION['droituser'] == 2)
                { ?>
            <a class="btn btn-success btn-lg" href="ajoutdocument.php?id=<?php echo $id;?>"><span class="glyphicon glyphicon-plus"></span> Ajouter un document</a></h1>
             <?php   } ?>
        ...
        <table class="table table-striped table-bordered avectri">
            ...
            $db = Database::connect();
            $caracteristique = $db->query('SELECT iddocument, document_nom, document_document, document_date FROM document WHERE idmachine = "'.$id.'"');
